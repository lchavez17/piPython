...
try:
    ...
    id_arduino=cur_arduinos.execute("""SELECT * FROM arduinos""")
    indice=1
    num_lineas=1
    sensor=1
    valor=2
    ...
    while indice<=id_arduino:
        ...
        if lines[0] == str(indice):
            ...
            for value in range(1,(len(lines))/2): #for para agregar los sensores y valores.. arreglar esta linea si un trama es mas chica que la otra truna
                cur_mediciones.execute("""INSERT INTO mediciones (id_arduino,id_sensor,valor) VALUES(%s,%s,%s)""",(lines[0],lines[sensor],lines[valor]))
                # print value
                db.commit()
    ...
    print "Reiniciando Valores de alarmas enviadas"
    db.commit()
    db.close()
